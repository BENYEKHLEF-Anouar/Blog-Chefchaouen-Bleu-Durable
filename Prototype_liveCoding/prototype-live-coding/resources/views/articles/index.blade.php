...
@section('content')
    <div class="container mx-auto p-4">
        <div class="bg-white shadow-md rounded-lg mb-6">
            <div class="px-6 py-4 bg-green-600  text-white font-bold rounded-t-lg">
                Articles List
            </div>
            <div class="p-6">
                @if (session('success'))
                            ...
                            <tr>
                                ...
                                <td class="py-2 px-4 border-b">
                                    @foreach ($article->categories as $category)
                                        <span class="inline-block bg-green-500 text-white text-xs px-2 py-1 rounded-full mr-1">{{ $category->name }}</span>
                                    @endforeach
                                </td>
                                <td class="py-2 px-4 border-b"><i class="fa-solid fa-bolt"></i> {{ $article->statut }}</td>
                                <td class="py-2 px-4 border-b"><i class="fa-solid fa-calendar"></i> {{ $article->created_at->format('d/m/Y') }}</td>
                                <td class="py-2 px-4 border-b">
